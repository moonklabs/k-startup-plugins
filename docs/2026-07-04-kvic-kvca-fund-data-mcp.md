...
1. 사용자 질문을 `investor_profile_lookup`, `startup_fund_search`, `new_fund_event_monitoring`, `founder_question_answering`, `raw_evidence_search` 중 하나로 분류한다.
2. 투자사명, 펀드명, 라운드, 섹터, 지역, TIPS 여부, 기간 조건을 추출한다.
3. 투자사/펀드 alias를 정규화한다.
4. `v_searchable_entities`와 전용 query를 통해 후보를 넓게 recall한다.
5. `search-contract.yaml`의 ranking model로 점수를 계산한다.
6. 결과마다 `evidence_status`, `why_ranked`, `source_url`, `hash`, `parser_warnings`, `caveats`를 붙인다.
7. 공식 근거가 부족하면 결과를 억지로 채우지 않고 `list_data_gaps`와 recommended imports를 표시한다.
