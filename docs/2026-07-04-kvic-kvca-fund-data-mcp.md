...
3. PDF/HWP/HWPX/HWPML/Office 같은 첨부 공시 원본은 해시와 출처를 남겨 evidence store에 보관하고, `kordoc` CLI/MCP로 변환한 파싱 결과는 별도 테이블로 분리한다.
...
| 패키지 | 역할 |
|---|---|
| `vc-fund-disclosure-core` | HTML/CSV parser, `kordoc` document adapter, 정규화, diff, evidence pack 생성 |
...
이 문서는 전략과 배경을 설명한다. 실제 구현자가 따라야 하는 canonical spec은 아래 파일들이다.

| 파일 | 역할 |
|---|---|
| `startup-fundraise/mcp/vc-fund-disclosure/README.md` | spec pack 개요와 구현 순서 |
| `startup-fundraise/mcp/vc-fund-disclosure/source-registry.yaml` | 수집 원천, 정책 상태, 필수 필드, 품질 체크 정의 |
| `startup-fundraise/mcp/vc-fund-disclosure/schema.sql` | canonical SQLite schema, index, view |
| `startup-fundraise/mcp/vc-fund-disclosure/seed-sources.sql` | fresh DB source seed |
| `startup-fundraise/mcp/vc-fund-disclosure/tool-contract.yaml` | MCP tool surface와 응답/display contract |
| `startup-fundraise/mcp/vc-fund-disclosure/data-trust-resolution-contract.yaml` | 신뢰 source tier, authoritative scope, 사용자 입력 resolution, answer gate 계약 |
| `startup-fundraise/mcp/vc-fund-disclosure/search-contract.yaml` | 자연어 검색, 랭킹, evidence status, empty state 계약 |
| `startup-fundraise/mcp/vc-fund-disclosure/server-cli-usage.md` | 스타트업 사용자용 MCP/CLI 소개와 명령 예시 |
...
| `startup-fundraise/mcp/vc-fund-disclosure/display-queries.sql` | investor profile, 신규 이벤트, fund search, guide search 표시 쿼리 |
| `startup-fundraise/mcp/vc-fund-disclosure/quality-checks.sql` | import 후 품질/누락/PII 후보 점검 쿼리 |

실제 구현 순서:

1. `schema.sql`로 DB를 만든다.
2. `seed-sources.sql`로 수집 source를 주입한다.
3. import adapter는 원본 파일/HTML을 `raw_artifacts`에 먼저 저장한다.
4. 정규화 row는 source snapshot/document/evidence edge로 연결한다.
...
7. 실제 구현 레포로 분리할 때는 `implementation-blueprint.md`의 `core + thin CLI/MCP adapters` 구조를 따른다.
...
5. `knowledge`/`retrieval`은 초기 창업자를 위한 `kordoc` 변환 PDF/HWP/HWPX/HWPML/Office 가이드 문서 검색, 용어 설명, 체크리스트 생성을 담당한다.
6. `fixtures`는 개인정보를 제거한 최소 HTML/PDF/HWP/HWPX 예제로만 구성한다.
...
- 사용자는 내부 코드나 파일 위치를 몰라도 자연어로 물을 수 있어야 한다.
- CLI 결과는 실제 예시처럼 표와 caveat를 함께 보여줘야 한다.
- 원격 MCP에서는 서버 파일시스템 접근 도구를 빼고, 로컬 MCP에서만 파일 경로 기반 import/parse를 제공한다.

VC 공시 MCP에 적용하면 초기 형태는 `vc-fund-disclosure-core`와 `vc-fund-disclosure-mcp`로 나눈다. P1 core는 KVIC/KVCA HTML/CSV snapshot import와 정규화를 담당하고, 표 snapshot으로 들어온 XLS/XLSX는 `unsupported_format`으로 안내한다. PDF/HWP/HWPX/HWPML/DOCX/XLS/XLSX 문서 import는 `kordoc` CLI/MCP adapter 단계로 둔다. MCP는 core를 호출해 검색, 신규 이벤트, evidence pack만 제공한다.

## 검색 정확도와 결과 계약

스타트업 사용자는 공시 사이트의 경로, 파라미터, 조합 유형을 모르는 상태에서 질문한다. 따라서 MCP의 핵심은 단순 DB 조회가 아니라 "질문 의도 분류 → recall → 랭킹 → 근거 설명 → data gap 안내"다.

검색 파이프라인:

1. 사용자 질문을 `investor_fund_holding`, `startup_fund_search`, `tips_signal`, `new_fund_event`, `founder_education` 중 하나로 분류한다.
2. 투자사명, 펀드명, 라운드, 섹터, 지역, TIPS 여부, 기간 조건을 추출한다.
3. 투자사/펀드 alias를 정규화하고 `entity_resolution_candidates`에 후보와 점수를 저장한다.
4. intent별 authoritative source matrix로 어떤 source가 있어야 verified answer가 가능한지 결정한다.
5. `v_searchable_entities`와 전용 query를 통해 후보를 넓게 recall한다.
6. `data-trust-resolution-contract.yaml`의 answer gate와 `search-contract.yaml`의 ranking model로 점수를 계산한다.
7. 결과마다 `resolution_status`, `evidence_status`, `source_trust_tier`, `authority_scope`, `why_ranked`, `source_url`, `hash`, `parser_warnings`, `caveats`를 붙인다.
8. 공식 근거가 부족하면 결과를 억지로 채우지 않고 `search_vc_database` 응답의 `data_gaps`와 `recommended_imports`를 표시한다.

결과 상태:

| 상태 | 사용자 표시 | 의미 |
|---|---|---|
| `verified_official` | 공식 근거 확인 | 공식 source snapshot/document와 정규화 row가 연결되고 critical quality flag가 없음 |
| `official_needs_review` | 공식 근거 있음, 검토 필요 | 공식 source는 있으나 parser warning 또는 review가 필요 |
| `guide_only` | 일반 가이드 | 창업자 guide 근거만 있음 |
| `user_note_only` | 사용자 메모 | private note만 있고 공식 근거는 없음 |
| `no_evidence` | 근거 없음 | 현재 로컬 DB에 근거가 없어 import 액션이 필요 |
...
- 사용자가 직접 내려받은 HTML 표 또는 CSV import
- 사용자가 직접 내려받은 PDF/HWP/HWPX/HWPML/DOCX/XLS/XLSX 공시 첨부파일 import. 파싱은 `kordoc` CLI/MCP adapter 경유
...
|---|---|---|---|---|
| `manual_snapshot_import` | ON | 낮음 | 사용자가 저장한 FundFinder/KVCA HTML, CSV를 import하고 XLS/XLSX는 CSV/HTML 재저장을 안내 | 안전한 기본값 |
| `watch_folder_import` | ON | 중간 | `~/Downloads/vc-disclosures`에 저장되는 PDF/HWP/HWPX/HWPML/DOCX/XLSX/HTML을 자동 import | 로컬 파일만 처리. 문서 파싱은 `kordoc` adapter 경유 |
...
| 첨부 공시 | 신규 펀드 결성, 조합 변경, 운용사 공시 PDF/HWP/HWPX/HWPML/DOCX/XLS/XLSX | 원본 파일 + `kordoc` Markdown/표 + 추출 이벤트 |
| 창업자 가이드 | 투자유치 절차, TIPS 안내, 투자계약 용어, IR/데이터룸 체크리스트 PDF/HWP/HWPX/HWPML/Office | 원본 파일 + `kordoc` 텍스트 청크 + 개념/체크리스트 카드 |
...
초기 투자유치 기업은 VC/AC 목록보다 먼저 "투자유치가 무엇이고 무엇을 준비해야 하는지"를 모르는 경우가 많다. 따라서 PDF/HWP/HWPX/HWPML/Office 자료를 로컬에 저장하고 `kordoc`으로 Markdown/표를 추출해 창업자 교육 코퍼스로 쓰는 것이 좋다.
...
- 원본 PDF/HWP/HWPX/HWPML/Office 문서는 개인 로컬 DB 보관함에만 둔다.
...
planned import adapter는 `vc-funds` 내부에서 포맷별 파서를 직접 구현하지 않고 `kordoc`을 호출한다.
...
MCP 연결 환경에서는 `kordoc`의 `parse_document`, `parse_table`류 도구를 사용해 Markdown/표와 parser warning을 받고, DB에는 원본 해시와 파싱 산출물을 분리 저장한다.

## DB 스키마 초안

아래 스키마는 설계 기록용이다. 실제 구현 기준은 `startup-fundraise/mcp/vc-fund-disclosure/schema.sql`을 우선한다.
...
tools:
  ...
  - name: import_kvic_snapshot
    input: { file_path: string, source_url?: string, captured_at?: string, group_code?: string, subcategory_code?: string }
    output:
      ok: boolean
      status: success | partial | unsupported_format
      source_id: string
      source_name: string
      source_url: string
      file: string
  ...
  - name: import_kvca_snapshot
    input: { file_path: string, source_url?: string, captured_at?: string, vc_name?: string }
    output: same envelope as import_kvic_snapshot
...
2. 실제 원본에서 개인정보를 제거한 최소 HTML/PDF/HWP/HWPX fixture를 만든다.
...
1. `import_kvic_snapshot`: 저장 HTML 또는 CSV를 받아 펀드 목록과 카테고리를 정규화한다. XLS/XLSX는 `unsupported_format`으로 반환한다.
   - `ASCT_CLSS_GRP_CD_FND`, `ASCT_CLSS_CD_FND`, 대분류명, 소분류명을 함께 저장한다.
   - 대분류와 소분류 prefix가 다를 수 있으므로 문자열 prefix 규칙으로 추론하지 않는다.
2. `import_kvca_snapshot`: 조합현황 목록 HTML 또는 CSV를 받아 조합 목록을 정규화한다. XLS/XLSX는 `unsupported_format`으로 반환한다.
...
1. `kordoc` CLI/MCP adapter를 기본 문서 파서로 둔다.
   - CLI: `npx -y kordoc <file>`
   - MCP: `parse_document`, `parse_table`류 도구
2. 지원 대상은 PDF, HWP, HWPX, HWPML, DOCX, XLS, XLSX를 포함한다.
3. `vc-funds` core 안에 PDF text parser, HWPX ZIP/XML parser, HWP binary 추측 파서를 직접 구현하지 않는다.
4. `kordoc`이 실패하거나 표/텍스트 품질이 낮으면 `parser_status=failed` 또는 `needs_review`, `parser_warnings`, 원본 해시를 저장하고 원본 확인을 요구한다.
5. 문서 파싱 결과에서 `신규 결성`, `출자`, `조합명`, `결성총액`, `운용사`, `등록일`, `만기일`, `투자분야` 패턴을 event candidate로 만든다.
6. confidence가 낮은 추출은 자동 확정하지 않고 `review_required`로 표시한다.

### Phase 4.5: 창업자 가이드 코퍼스

1. `import_founder_guide_document`로 투자유치/TIPS/IR/데이터룸/투자계약 안내 PDF/HWP/HWPX/HWPML/Office 문서를 import하고 `kordoc` 산출물을 chunking한다.
...
4. `kordoc` parser warning이 있으면 MCP가 확정적 표현을 쓰지 않도록 한다.
...
- PDF/HWP 문서는 표 구조와 스캔 품질이 제각각이므로 `kordoc` parser warning, extraction quality, 원본 확인 경로를 항상 남긴다.
