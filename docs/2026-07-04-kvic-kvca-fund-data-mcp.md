...
VC 공시 MCP에 적용하면 초기 형태는 `vc-fund-disclosure-core`와 `vc-fund-disclosure-mcp`로 나눈다. P1 core는 KVIC/KVCA HTML/CSV snapshot import와 정규화를 담당하고, XLS/XLSX는 `unsupported_format`으로 안내한다. PDF/HWP/HWPX 문서 import는 별도 adapter 단계로 둔다. MCP는 core를 호출해 검색, 신규 이벤트, evidence pack만 제공한다.
