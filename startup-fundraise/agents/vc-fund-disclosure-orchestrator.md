...
> **아카이브 (사용 금지)**: `mcp/vc-fund-disclosure/runtime/`의 Node 진입점은 canonical 구현 전환 이전 스텁입니다. 설치·검증은 항상 위 `install.sh` + `vc-funds` 바이너리 경로를 씁니다. 이 스텁은 최신 스키마·데이터 롤업·부트스트랩을 반영하지 않습니다.
...
> **주의**: `mcp/vc-fund-disclosure/*.yaml`(tool-contract, search-contract, data-trust-resolution-contract)과 `implementation-blueprint.md`는 canonical 전환 이전 설계 초안입니다. 계약 YAML의 canonical 사본은 `moonklabs/vc-fund-disclosure` 리포의 `docs/contracts/`로 이관되었으므로, 도구 계약을 확인할 때는 그쪽을 우선 참조합니다.
...
```bash
git diff --check
ruby -e 'require "yaml"; %w[docs/tools-capability-matrix.yaml startup-fundraise/mcp/vc-fund-disclosure/source-registry.yaml startup-fundraise/mcp/vc-fund-disclosure/tool-contract.yaml startup-fundraise/mcp/vc-fund-disclosure/search-contract.yaml startup-fundraise/mcp/vc-fund-disclosure/data-trust-resolution-contract.yaml].each { |f| YAML.load_file(f); puts "yaml ok #{f}" }'
