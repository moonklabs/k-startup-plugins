...
repo-local draft 런타임 점검은 설치 UX 대신 Node 진입점을 직접 사용합니다.
...
현재 P0 런타임에는 `import guide`, `guide-source add`, `ask` 명령이 없습니다. 이 흐름은 `data-trust-resolution-contract.yaml`과 `implementation-blueprint.md`의 planned surface로만 다루고, 사용자에게는 `NOT_READY`로 보고합니다.

planned guide/document adapter는 `kordoc`을 기본 파서로 사용합니다.
...
현재 P0 런타임에는 guide source 등록 명령이 없습니다. 링크가 죽었더라도 사용자가 로컬 파일을 갖고 있으면 향후 guide/document adapter에서 `source_url`을 보존한다는 설계 원칙만 기록합니다.
