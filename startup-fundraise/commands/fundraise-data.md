...
> **`.kb/`와의 역할 분담:** startup-apply의 `/kb-init`이 만드는 `.kb/`는 **회사 사실(팩트)** — 지표·시장·팀·실적 — 의 단일 진실 공급원이고, 이 커맨드가 정리하는 작업공간은 **활동 기록** — 파이프라인·미팅 노트·데이터룸·공시 근거 — 전담입니다. 두 플러그인을 함께 쓴다면 `/kb-init`을 먼저 실행하고, 여기서 발견된 IR·과거 사업계획서는 `/kb-init`의 입력으로 넘기세요. 회사 사실이 갱신되면 `/kb-update`로 KB를 동기화합니다.
