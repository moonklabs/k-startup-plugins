...
├─────────────────────────────────────────────────────────────────┤
│  단독 사용 (항상 작동)                                             │
│  ✓ vc-funds CLI 설치 상태 점검 (vc-funds doctor)                  │
│  ✓ 미설치 시 install.sh → setup → 데이터 부트스트랩 순서 안내      │
│  ✓ 로컬 SQLite DB, watch folder, guide library 구조 설계          │
│  ✓ 수집 경계 정책 점검 (robots 동의, 재배포, crawler 금지)         │
├─────────────────────────────────────────────────────────────────┤
│  강화 모드 (도구 연결 시)                                          │
│  + ~~fund disclosure: 설치 후 search/resolve/import 근거 조회      │
│  + kordoc MCP: PDF/HWP/HWPX/Office 창업자 가이드 파싱              │
└─────────────────────────────────────────────────────────────────┘
```
...
이 커맨드 실행 시 `vc-fund-disclosure-mcp`, `fundraise-data` 스킬이 자동 활성화됩니다.
