...
1. 질문 의도를 `startup_fund_search`로 분류합니다.
2. 회사 단계/섹터를 FundFinder 코드 후보와 local fund focus로 변환합니다.
3. KVIC/KVCA/TIPS/documents/user notes를 분리 검색합니다.
4. 결과마다 `evidence_status`, `why_ranked`, `source_url`, `hash`, `parser_warnings`, `data_gaps`를 붙입니다.
5. 근거가 없으면 빈 결과를 숨기지 않고 필요한 snapshot/document import를 안내합니다.
