...
description: 한국 정부/민간 지원사업 공고 체계, 평가 기준, 주요 프로그램 특성을 안내합니다. TIPS는 운영사의 투자확약/추천이 선행되는 투자자 주도형 프로그램으로 다룹니다. "지원사업", "정부 과제", "TIPS", "팁스 운영사", "예비창업패키지", "창업성장기술개발", "AI바우처", "공고 분석", "apply-find", "apply-check" 등의 맥락에서 자동 활성화됩니다.
---
...
| 중앙정부 | 과학기술정보통신부 | AI 바우처(NIPA 시행), ICT R&D |
...
| 공공기관 | NIPA (정보통신산업진흥원) | AI 바우처 시행, SW·클라우드 지원사업 |
...
- **지원 규모:** 창업팀당 최대 8억원 내외(운영사 투자, R&D, 창업자금, 해외마케팅 등 연계)
- **대상:** 기술 기반 스타트업 (업력 7년 이내)
- **핵심:** 민간 운영사가 먼저 투자/투자확약 → 운영사 추천 → 정부 R&D 등 매칭
- **선발 과정:** 운영사 투자심사 → 투자확약 창업팀 추천 → 서면/대면 선정평가
- **강조 포인트:** 기술 혁신성, 글로벌 확장 가능성
- **운영사 선택:** 창업자가 임의로 고르는 지원기관이 아니라, 투자 가능성이 있는 운영사의 펀드 목적/추천 가능성/실적을 함께 봐야 함

**TIPS 판단 규칙:**
- `투자 적합도`와 `TIPS 추천 가능성`을 분리해서 본다.
- 운영사 여부, 접수 경로, 추천 가능 TO/일정, 최근 추천/선정 실적을 확인한다.
- KVIC/KVCA 근거로 해당 운영사의 펀드 목적이 회사 단계/섹터와 맞는지 확인한다.
- LinkedIn/블로그/뉴스의 공개 발언은 `업계 신호`로 쓰고 공식 근거와 분리한다.
- startup-fundraise 플러그인을 함께 설치했다면, 상세 투자자 관점 설계는 그 플러그인의 `Korea TIPS Investor-Led Process` reference(deal-sourcing 스킬)를 따른다. 미설치 시에는 이 스킬의 TIPS 항목과 웹 검색 기반으로 진행한다.
