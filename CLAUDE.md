...
이 레포지토리는 Claude Code 플러그인 마켓플레이스 모음입니다. 두 개의 플러그인이 포함되어 있습니다:

- `startup-fundraise` — 스타트업 창업자의 VC/AC 투자 유치 일일 루틴 지원
- `startup-apply` — 정부/민간 지원사업 공고 소싱, 사업계획서 자동 작성, HWPX 출력

## 플러그인 설치 및 테스트

```bash
# 개발용 마켓플레이스 등록 (로컬 경로)
/plugin marketplace add /path/to/k-startup-plugins

# 플러그인 설치
/plugin install startup-fundraise@startup-plugins
/plugin install startup-apply@startup-plugins
...
```json
{
  "plugins": [
    ...
    { "name": "startup-apply", "source": "./startup-apply" },
...
플러그인 변경 작업의 계획(Linear 등록)부터 검증·버전 범프·이중 배포(Claude+Codex)·마감까지는 **`plugin-release` 프로젝트 스킬**(`.claude/skills/plugin-release/SKILL.md`)의 9단계 루프를 따릅니다. "배포해줘", "버전 올려서 재설치", "Linear 등록하고 진행" 같은 요청 시 이 스킬을 사용하세요.
